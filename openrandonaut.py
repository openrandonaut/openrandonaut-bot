...
def main_generate_location(start_lat, start_lon, radius, num_points):
    ...
    # return f"{max_dense_lon}, {max_dense_lat}"
...
# def coordinates():
#     lat = float(request.args.get("start_lat"))
#     long = float(request.args.get("start_lon"))
#     radius = int(request.args.get("radius"))
#     num_points = int(request.args.get("num_points"))
#     # globe.is_land
#     retries = 0
#
#     lat, long = main_generate_location(lat, long, radius, num_points)
#     if globe.is_land(lat, long):
#
#         return f"{lat}, {long}"
#     else:
#         raise Exception("Got non-land coordinates. Try again.")
#
#
# if __name__ == "__main__":
#     app.run(debug=True)
#
# ## main_generate_location(56.162927, 10.206036, 5000, 2048)
